    ...
    if content in property_node['name'].values: #如果当前处理的值在已有的dataframe已经存在
        ...
        for i in relative_content_line['property_id'].values:
            if i in used_node_flag: #如果该id已经被处理过，则查看下一个
                ...
                merge = '''match (from:yuanshi{name:"%s",id:"%s"}) merge (from)-[r:rel{pro1:"%s"}]->(to:attribute{name:"%s",id:"%s"})''' %(yuanshi_name, "yuanshi_"+str(yuanshi_name_id), word, content, i)
                test_graph.run(merge)
                break
        if flag==0: #表示所有该值对应的行都被处理过
            merge = '''match (from:yuanshi{name:"%s",id:"%s"}) merge (from)-[r:rel{pro1:"%s"}]->(to:attribute{name:"%s",id:"%s"})''' %(yuanshi_name, "yuanshi_"+str(yuanshi_name_id), word, content, "property_"+str(count_property_id))
            test_graph.run(merge)
...
def deal_yuanshinameid_and_in_process_entrnce(every_baike_json, json_file):
    ...
    count_property_id += 1
    merge = '''merge (from:yuanshi{name:"%s",id:"%s"})-[r:rel{pro1:"中文名"}]->(to:attribute{name:"%s",id:"%s"})''' %(yuanshi_name, count_yuanshi_temp_id, yuanshi_name, count_property_temp_id)
    test_graph.run(merge)
    process_entrace(key_list, yuanshi_name_id)

def main(wait_deal_json, name):
    global yuanshi_node
    global property_node
    global relation
